...
export const TableFooter = ({ children, className, ...props }) => (
  <tfoot
    className={`border-t bg-gray-100/50 dark:bg-gray-700/50 font-medium [&>tr]:last:border-b-0 ${
      className || ""
    }`}
    {...props}
  >
...
export const TableRow = ({ children, className, ...props }) => (
  <tr
    className={`border-b transition-colors data-[state=selected]:bg-gray-100 hover:bg-gray-50/50 dark:hover:bg-gray-800/50 ${
      className || ""
    }`}
    {...props}
  >
...
const getStatusBadge = (status) => {
  ...
  switch (status) {
    ...
    case "Pending":
      return (
        <span
          className={`${badgeClass} bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300`}
        >
          {status}
        </span>
      );
    ...
    default:
      return (
        <span
          className={`${badgeClass} bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300`}
        >
          {status}
        </span>
      );
...
const InvoiceTable = () => {
  const totalAmount = MOCK_INVOICES.reduce(
      ...
      <Table className="border dark:border-gray-700 rounded-lg">
        ...
        <TableHeader>
          <TableRow className="bg-gray-50 dark:bg-gray-700">
            <TableHead className="w-[100px] font-bold text-gray-900 dark:text-white">
              Invoice
            </TableHead>
        ...
        <TableFooter>
          <TableRow>
            ...
            <TableCell className="text-right font-extrabold text-lg text-gray-900 dark:text-white">
